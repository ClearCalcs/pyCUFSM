...
def prevals():
    ...
    nodes =np.array([[0, 2.5, 0.773, 1, 1, 1, 1, 1], [1, 2.5, 0, 1, 1, 1, 1, 1],
                [2, 1.25, 0, 1, 1, 1, 1, 1], [3, 0, 0, 1, 1, 1, 1, 1],
                [4, 0, 3, 1, 1, 1, 1, 1], [5, 0, 6, 1, 1, 1, 1, 1],
                [6, 0, 9, 1, 1, 1, 1, 1], [7, 1.25, 9, 1, 1, 1, 1, 1],
                [8, 2.5, 9, 1, 1, 1, 1, 1], [9, 2.5, 8.227, 1, 1, 1, 1, 1]])
...
class Preprocess:
    ...
    def wprops(self, props, m):
        ...
        for i in range(self.m):
            if(i<len(self.props)):
                for j in range(6):
                    self.mitems[i].append(widgets.FloatText(value=self.props[i,j], layout = widgets.Layout(width='70px')))
                prop[i] = widgets.GridBox(self.mitems[i], layout=widgets.Layout(grid_template_columns="repeat(6, 50px[col-start])"))
            if(i>=len(self.props)):
                for j in range(6):
                    self.mitems[i].append(widgets.FloatText(value=self.props[i-1,j], layout = widgets.Layout(width='70px')))
                prop[i] = widgets.GridBox(self.mitems[i], layout=widgets.Layout(grid_template_columns="repeat(6, 50px[col-start])"))
        matr = widgets.VBox([prop[j] for j in range(m)])
        brow = widgets.HBox([self.ADDMAT, self.DELMAT])
        self.rowm = widgets.VBox([matTitle, matlabel, matr, brow], layout= widgets.Layout(border = 'solid 1px black', width = '35%'))
    ...
    def welems(self, elements, e):
        ...
        self.ADDELEM = widgets.Button(description="Add Element", layout= widgets.Layout(border = 'solid 1px black', width = '120px'))
        self.DELELEM = widgets.Button(description="Remove Element", layout= widgets.Layout(border = 'solid 1px black', width = '120px'))
    ...
    def wflag(self, flag):
        ...
        Flagtext = ['node', 'elem', 'mat', 'stress', 'stresspic', 'coord', 'constraints', 'springs', 'origin', 'propaxis']
        self.flags = []
    ...
    def wBound_Cond(self):
        ...
        BCtext = ['simple-simple', 'clamped-clamped', 'simple-clamped', 'clamped-free', 'clamped-guided']
        self.bc_widget = widgets.Dropdown(options = [(BCtext[i], i) for i in range(len(BCtext))], value = 0, description = 'Boundary Conditions')
