...
def prevals():
    ...
    nodes = np.array([[0, 2.5, 0.773, 1, 1, 1, 1, 1], [1, 2.5, 0, 1, 1, 1, 1, 1],
                      ...
                      [4, 0, 3, 1, 1, 1, 1, 1], [5, 0, 6, 1, 1, 1, 1, 1], [6, 0, 9, 1, 1, 1, 1, 1],
                      [7, 1.25, 9, 1, 1, 1, 1, 1], [8, 2.5, 9, 1, 1, 1, 1, 1],
...
class Preprocess:
    ...
    def wprops(self, props, m):
        ...
        self.DELMAT = widgets.Button(
            description="Remove Material", layout=widgets.Layout(border='solid 1px black')
        )
        matlabel = widgets.GridBox([widgets.Label(value=mattext[j]) for j in range(6)],
        ...
        for i in range(self.m):
            ...
            if (i >= len(self.props)):
                for j in range(6):
                    self.mitems[i].append(
                        widgets.FloatText(
                            value=self.props[i - 1, j], layout=widgets.Layout(width='70px')
                        )
                    )
                prop[i] = widgets.GridBox(
                    self.mitems[i],
    ...
    def wnodes(self, nodes, n):
        ...
        self.rnode = widgets.VBox([nodTitle, nlabel, noder0, brow],
    ...
    def welems(self, elements, e):
        ...
        elabel = widgets.GridBox([widgets.Label(value=elemtext[j]) for j in range(5)],
                                 layout=widgets.Layout(
        ...
        self.relem = widgets.VBox([elTitle, elabel, elemr, brow],
                                  layout=widgets.Layout(border='solid 1px black'))
    ...
    def wflag(self, flag):
        ...
        Flagtext = [
            'node', 'elem', 'mat', 'stress', 'stresspic', 'coord', 'constraints', 'springs',
            'origin', 'propaxis'
        ]
        self.flags = []
        ...
        self.Submit = widgets.Button(
            description="Plot", layout=widgets.Layout(border='solid 1px black', width='150px')
        )
    ...
    def wBound_Cond(self):
        ...
        BCtext = [
            'simple-simple', 'clamped-clamped', 'simple-clamped', 'clamped-free', 'clamped-guided'
        ]
