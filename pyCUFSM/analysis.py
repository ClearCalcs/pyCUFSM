...
def bc_i1_5(b_c, m_i, m_j, length):
    ...
    if b_c == 'S-S':
        ...
        if m_i == m_j:
            ...
            i_4 = ((m_i - 1/2)**2 + 1/4)**2*np.pi**4/4/length**3+(m_i - 1/2)**2*np.pi**4/4/length**3
    ...
    for i in range(0, total_m):
        for j in range(0, total_m):
            ...
            k_3_matrix[4*n_nodes*i+skip+(node_i+1)*2-2:4*n_nodes*i+skip+(node_i+1)*2,
                       4*n_nodes*j+skip+(node_i+1)*2-2:4*n_nodes*j+skip+(node_i+1)*2] = kg33
            k_3_matrix[4*n_nodes*i+skip+(node_i+1)*2-2:4*n_nodes*i+skip+(node_i+1)*2,
                       4*n_nodes*j+skip+(node_j+1)*2-2:4*n_nodes*j+skip+(node_j+1)*2] = kg34
            k_3_matrix[4*n_nodes*i+skip+(node_j+1)*2-2:4*n_nodes*i+skip+(node_j+1)*2,
                       4*n_nodes*j+skip+(node_i+1)*2-2:4*n_nodes*j+skip+(node_i+1)*2] = kg43
            k_3_matrix[4*n_nodes*i+skip+(node_j+1)*2-2:4*n_nodes*i+skip+(node_j+1)*2,
                       4*n_nodes*j+skip+(node_j+1)*2-2:4*n_nodes*j+skip+(node_j+1)*2] = kg44

            k_3_matrix[4*n_nodes*i+(node_i+1)*2-2:4*n_nodes*i+(node_i+1)*2,
                       4*n_nodes*j+skip+(node_i+1)*2-2:4*n_nodes*j+skip+(node_i+1)*2] = kg13
            k_3_matrix[4*n_nodes*i+(node_i+1)*2-2:4*n_nodes*i+(node_i+1)*2,
                       4*n_nodes*j+skip+(node_j+1)*2-2:4*n_nodes*j+skip+(node_j+1)*2] = kg14
            k_3_matrix[4*n_nodes*i+(node_j+1)*2-2:4*n_nodes*i+(node_j+1)*2,
                       4*n_nodes*j+skip+(node_i+1)*2-2:4*n_nodes*j+skip+(node_i+1)*2] = kg23
            k_3_matrix[4*n_nodes*i+(node_j+1)*2-2:4*n_nodes*i+(node_j+1)*2,
                       4*n_nodes*j+skip+(node_j+1)*2-2:4*n_nodes*j+skip+(node_j+1)*2] = kg24
    ...
    for i in range(0, total_m):
        for j in range(0, total_m):
            ...
            k_2_matrix[4*n_nodes*i+skip+(node_i+1)*2-1:4*n_nodes*i+skip+(node_i+1)*2,
                       4*n_nodes*j+skip+(node_i+1)*2-1:4*n_nodes*j+skip+(node_i+1)*2] = k33
            if node_j != 0:
                k_2_matrix[4*n_nodes*i+skip+(node_i+1)*2-1:4*n_nodes*i+skip+(node_i+1)*2,
                           4*n_nodes*j+skip+(node_j+1)*2-1:4*n_nodes*j+skip+(node_j+1)*2] = k34
                k_2_matrix[4*n_nodes*i+skip+(node_j+1)*2-1:4*n_nodes*i+skip+(node_j+1)*2,
                           4*n_nodes*j+skip+(node_i+1)*2-1:4*n_nodes*j+skip+(node_i+1)*2] = k43
                k_2_matrix[4*n_nodes*i+skip+(node_j+1)*2-1:4*n_nodes*i+skip+(node_j+1)*2,
                           4*n_nodes*j+skip+(node_j+1)*2-1:4*n_nodes*j+skip+(node_j+1)*2] = k44

            k_2_matrix[4*n_nodes*i+(node_i+1)*2-1:4*n_nodes*i+(node_i+1)*2,
                       4*n_nodes*j+skip+(node_i+1)*2-1:4*n_nodes*j+skip+(node_i+1)*2] = k13
            if node_j != 0:
                k_2_matrix[4*n_nodes*i+(node_i+1)*2-1:4*n_nodes*i+(node_i+1)*2,
                           4*n_nodes*j+skip+(node_j+1)*2-1:4*n_nodes*j+skip+(node_j+1)*2] = k14
                k_2_matrix[4*n_nodes*i+(node_j+1)*2-1:4*n_nodes*i+(node_j+1)*2,
                           4*n_nodes*j+skip+(node_i+1)*2-1:4*n_nodes*j+skip+(node_i+1)*2] = k23
                k_2_matrix[4*n_nodes*i+(node_j+1)*2-1:4*n_nodes*i+(node_j+1)*2,
                           4*n_nodes*j+skip+(node_j+1)*2-1:4*n_nodes*j+skip+(node_j+1)*2] = k24
