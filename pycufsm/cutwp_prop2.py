    ...
    if section == 'open':
        ...
        for m in range(nele):
            ...
            while (i < len(ends) - 1 and
                   ((np.any(w[:, 0] == ends[i, 0]) and np.any(w[:, 0] == ends[i, 1])) or
                    (not (np.any(w[:, 0] == ends[i, 0])) and (not np.any(w[:, 0] == ends[i, 1]))))):
                i = i + 1
            sn = int(ends[i, 0]) - 1
            fn = int(ends[i, 1]) - 1
            p = ((coord[sn, 0] - xc) * (coord[fn, 1] - yc) - (coord[fn, 0] - xc) *
                 (coord[sn, 1] - yc)) / L[i]
            if w[sn, 0] == 0:
                w[sn, 0] = sn + 1
                w[sn, 1] = w[fn, 1] - p * L[i]
                ...
                w[fn, 1] = w[sn, 1] + p * L[i]
            Iwx = Iwx + (
                1 / 3 * (w[sn, 1] * (coord[sn, 0] - xc) + w[fn, 1] * (coord[fn, 0] - xc)) + 1 / 6 *
                (w[sn, 1] * (coord[fn, 0] - xc) + w[fn, 1] * (coord[sn, 0] - xc))
            ) * t[i] * L[i]
            Iwy = Iwy + (
                1 / 3 * (w[sn, 1] * (coord[sn, 1] - yc) + w[fn, 1] * (coord[fn, 1] - yc)) + 1 / 6 *
                (w[sn, 1] * (coord[fn, 1] - yc) + w[fn, 1] * (coord[sn, 1] - yc))
            ) * t[i] * L[i]
        if (Ix*Iy - Ixy**2) != 0:
        ...
        for m in range(nele):
            ...
            sn = int(ends[i, 0]) - 1
            fn = int(ends[i, 0]) - 1
            po = ((coord[sn, 0] - xs) * (coord[fn, 1] - ys) - (coord[fn, 0] - xs) *
                  (coord[sn, 1] - ys)) / L[i]
            if w[sn, 0] == 0:
                w[sn, 0] = sn + 1
        ...
        for i in range(nele):
            ...
            x2 = coord12[fn, 0]
            y2 = coord12[fn, 1]
            B1 = B1 + ((y1+y2) * (y1**2 + y2**2) / 4 +
                       (y1 * (2 * x1**2 + (x1 + x2)**2) + y2 * (2 * x2**2 +
                                                                (x1 + x2)**2)) / 12) * L[i] * t[i]
            B2 = B2 + ((x1+x2) * (x1**2 + x2**2) / 4 +
                       (x1 * (2 * y1**2 + (y1 + y2)**2) + x2 * (2 * y2**2 +
                                                                (y1 + y2)**2)) / 12) * L[i] * t[i]
        B1 = B1/I1 - 2 * s12[1]
        B2 = B2/I2 - 2 * s12[0]
