...
cpdef np.ndarray klocal(float stiff_x, float stiff_y, float nu_x, float nu_y, float bulk, float thick, float length, float b_strip, str b_c, list m_a):
    ...
    cdef float e_1 = stiff_x / (1 - nu_x*nu_y)
    cdef float e_2 = stiff_y / (1 - nu_x*nu_y)
    cdef float d_x = stiff_x * thick**3 / (12 * (1 - nu_x*nu_y))
    cdef float d_y = stiff_y * thick**3 / (12 * (1 - nu_x*nu_y))
    cdef float d_1 = nu_x * stiff_y * thick**3 / (12 * (1 - nu_x*nu_y))
    cdef float d_xy = bulk * thick**3 / 12
...
cdef bc_i1_5(str b_c, float m_i, float m_j, float length):
...
cpdef kglocal(float length, float b_strip, float ty_1, float ty_2, str b_c, list m_a):
...
cpdef trans(float alpha, np.ndarray k_local, np.ndarray kg_local, list m_a):
...
def assemble(np.ndarray k_global, np.ndarray kg_global, np.ndarray k_local, np.ndarray kg_local, int node_i, int node_j, int n_nodes, list m_a):
